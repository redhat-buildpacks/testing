...
* <<4-RHTAP,Red Hat Trusted Application Pipeline>>
* <<5-shipwright,Shipwright>>

== Project Compatibility Matrix

|===
| Version | Tool/Feature | Compatible | Note
...
| https://github.com/paketo-buildpacks/builder-jammy-tiny[0.0.175]
| Paketo Builder Tiny (jammy)
| Yes
| Change this builder image with the ubi image
...
Do the build using as builder image `paketobuildpacks/builder-jammy-tiny:0.0.175` and where you pass the needed `+BP_***+` env variables in order to configure properly the Quarkus maven build:
