...
| Lifecycle
| https://github.com/buildpacks/lifecycle/releases/tag/v0.18.2[0.18.2]
| Yes
| -
...
| Pack cli
| https://github.com/buildpacks/pack/releases/tag/v0.32.1[v0.32.1]
| Yes
| -

| Paketo Builder Tiny (jammy) builder (DEPRECATED)
| https://github.com/paketo-buildpacks/builder-jammy-tiny/releases/tag/v0.0.211[0.0.211]
| Yes
| Package lifecycle 0.18.2

| Paketo community UBI builder
| https://github.com/paketo-community/builder-ubi-base/releases/tag/v0.0.24[0.0.24]
| Yes
| Include Node.js, Quarkus Java buildpacks.

| Paketo Node.js Extension for ubi
| https://github.com/paketo-community/ubi-nodejs-extension/releases/tag/v0.1.0[0.1.0]
| Yes
| -
...
To build the container image, do the build using as builder image `paketobuildpacks/builder-jammy-tiny:0.0.211` and pass SOME `+BP_***+` env variables in order to configure properly the Quarkus maven build:
...
The https://hub.docker.com/r/paketobuildpacks/builder/tags[paketo builder image] version `0.0.211` https://github.com/paketo-buildpacks/builder-jammy-tiny/releases/tag/v0.0.211[supports]:
...
BUILDER_IMAGE=paketobuildpacks/builder-jammy-tiny:0.0.211
...
NOTE: You can test different pipelineRuns using our bash script: ./scripts/play-with-tekton ;-)
...
As the Paketo builder images are quite big, we suggest to relocate them to the kind registry using the https://carvel.dev/imgpkg/docs/v0.37.x/install/[imgpkg] tool:

[,bash]
----
BUILDER_VERSION=0.0.211
imgpkg copy -i docker.io/paketobuildpacks/builder-jammy-tiny:$BUILDER_VERSION --to-tar ./k8s/builder-$BUILDER_VERSION.tar
