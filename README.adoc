...
Do the build using as builder image `paketobuildpacks/builder:0.1.361-tiny` and where you pass the needed `+BP_***+` env variables in order to configure
...
mvn package \
 ...
 -Dquarkus.buildpack.jvm-builder-image=paketobuildpacks/builder:0.1.361-tiny \
...
pack build ${REGISTRY_HOST}/quarkus-hello:1.0 \
     --builder paketobuildpacks/builder:0.1.361-tiny \
...
LIFECYCLE_IMAGE=buildpacksio/lifecycle:0.16.5
